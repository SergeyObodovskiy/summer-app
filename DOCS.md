...
- `validation.ts` — `validateGoalTitle()` + `GOAL_TITLE_MIN`: правила для пользовательского ввода (см. §8.6).
...
| `done?` | выполнена ли цель/задача |
| `doneAt?` | время выполнения (ms) — ставится при `done=true`, очищается при снятии |
...
### 8.6 Валидация названий
`validateGoalTitle(raw)` из `@summer/domain` (`validation.ts`) — чистая, общая для web и mobile. Обрезает пробелы и проверяет:
- **непустоту** — один пробел больше не создаёт цель (исходный баг);
- **отсутствие управляющих символов** (C0/DEL/C1: переносы, табы и пр. — приходят только вставкой); буквы любых языков, цифры, пунктуация и эмодзи разрешены;
- **минимум `GOAL_TITLE_MIN` (= 2) символов** после обрезки.

Возвращает `{ ok, value, error? }`, где `value` — обрезанная строка для сохранения.

Применение в `apps/next/features/goals.tsx`:
- **Фантомная строка** (создание новой цели): цель создаётся только при `ok`; невалидный ввод (пробел, один символ) удерживается в поле, ничего не создавая. На явный сабмит (**Enter**) с невалидным вводом — предупреждение под полем; **Esc** очищает.
- **Существующие строки**: на `blur` с непустым, но невалидным содержимым — мягкое предупреждение под строкой (`aria-invalid` + текст).
- **Пустые строки не persist'ятся.** Enter по дизайну создаёт пустую строку (скаффолдинг), но пустая бездетная строка **удаляется при потере фокуса** (`onLeaveEmpty` → `removeGoal`), а строки с детьми не трогаются (удаление каскадно снесло бы поддерево). Подстраховка: при уходе с экрана (`unmount`) удаляются все бездетные пустые цели — на случай, если строку оставили пустой и закрыли вкладку без blur.

Граница: валидация гейтит **создание**, предупреждает при редактировании и подчищает пустые строки, но не блокирует сам набор (иначе сломался бы Enter→пустая строка→печать).

### 8.7 Выполнение (чекбоксы)
У каждой строки (цели и задачи на любом уровне) — чекбокс. Действие стора `toggleGoalDone(id, done?)` переключает `done` и проставляет/очищает `doneAt` (время выполнения), бампая `clock["goalItem:<id>"]` — синхронизируется тем же пер-сущностным LWW, что и сама цель (см. §7.2), отдельный таймстамп не нужен.

UI (`goals.tsx`): выполненная строка — зачёркнутый приглушённый текст + компактная отметка времени (`12 июн, 14:30`); позиция в дереве не меняется. Чекбоксы независимы (отметка родителя не каскадит на детей). В шапке — счётчик «Выполнено N из M» по непустым целям.

Основа под будущий **глобальный прогресс**: `done`/`doneAt` лежат в общем состоянии и синкаются, так что агрегаты (доля выполненного, история по датам) можно считать селекторами без изменения модели.
