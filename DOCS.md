...
- `store.ts` — `createAppStore(kv)`: Zustand + persist. Все действия бампают `clock`. Действие `mergeRemote` сливает облако без потерь.
...
- `SyncBridge` — берёт конфиг из настроек/env и подключает `useSync`.
...
- `workout:<id>`, `meal:<id>` — пер-сущностный LWW (наличие/удаление тоже по таймстампу — «tombstone», чтобы старый клиент не воскрешал удалённое);
