...
- **Существующие строки**: на `blur` с непустым, но невалидным содержимым — мягкое предупреждение под строкой (`aria-invalid` + текст). **Пустая строка валидна** — это скаффолдинг outline-редактора (Enter создаёт пустую, Backspace удаляет), её не блокируем.

Граница: валидация гейтит **создание** и предупреждает при редактировании, но не блокирует набор в существующих строках (иначе сломался бы Enter→пустая строка→печать).
