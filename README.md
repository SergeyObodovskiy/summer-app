...
- **Экран «Цели»** — глобальные списки целей: по категориям («Туризм», «Стройка дома», …) или общим списком без категории; добавление/удаление из UI, необязательная заметка к цели. Данные в `AppState.goals`, синхронизируются как остальное состояние (LWW по `goalItem:<id>`). Пока только web (`apps/next/features/goals.tsx`).
