...
- **Экран «Цели»** — глобальные цели в духе milestone: создаёшь цель («Съездить в Японию»), внутрь добавляешь подцели («побывать в Токио», «побывать в Хоккайдо»); цель без подцелей — просто обычная цель. Подцель ссылается на родителя через `parentId`, удаление цели каскадно удаляет подцели. Добавление/удаление из UI, необязательные заметки на обоих уровнях. Данные в `AppState.goals`, синхронизируются как остальное состояние (LWW по `goalItem:<id>`). Пока только web (`apps/next/features/goals.tsx`).
