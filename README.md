...
- **Экран «Цели»** — outline-редактор в духе Apple Notes: каждая строка всегда редактируема, Enter создаёт строку ниже, Tab/Shift+Tab меняют вложенность (неограниченная глубина), Backspace на пустой строке удаляет её, ✎/✕ по наведению (заметка / удалить поддерево). Цели — верхний уровень, задачи — вложенные строки (`parentId`); порядок строк хранится в `order` (fractional ranking), удаление каскадное. Данные в `AppState.goals`, синхронизируются как остальное состояние (LWW по `goalItem:<id>`). Пока только web (`apps/next/features/goals.tsx`).
